...
public class ClientHandlerThread extends Thread implements NotifyListener {
	...
	// 게이트웨이 재시작 전송 결과
	private byte isGatewayRestartResult = 0x00;
	// 시스템 리부팅 전송 결과
	private byte isSystemRebootingResult = 0x00;
	// IF Server 주소변경 결과
	private byte isIFServerChangeStat = 0x00;
	// IF Server 주소변경 결과 bytes
	private byte[] ifServerChangeResult;
	// 시스템 리부팅 전송 결과
	private byte isIOBoardResetResult = 0x00;
	// 패치파일 전송 결과
	private byte isFileUpdateResult = 0x00;
     ...
     */
    @Override
    public boolean notifyFileUpdate(final NotifyEvent evt){
    	...
    	synchronized(writeLock){
    		...
    		if(storeCD.equals(evt.getStoreCD()) && gwID.equals(evt.getGwID())){
    			...
    			try{
    				...
    				isFileUpdateResult = 0x00;
    				
					write(evt.getData(), false);
					
					for(short i = 0; i < 20; i++){
						
						if(isFileUpdateResult == 0x00){
							Thread.sleep(1000);
						}else{
							
							if(isFileUpdateResult == NORMAL){
								flag = true;
							}
	 ...
	 */
    @Override
	public boolean notifyGatewayRestart(NotifyEvent evt){
    	...
    	synchronized(writeLock){
			...
			if(storeCD.equals(evt.getStoreCD()) && gwID.equals(evt.getGwID())){
				...
				try{
					...
					isGatewayRestartResult = 0x00;
					
					write(evt.getData(), false);
					
					for(short i = 0; i < 20; i++){
						
						if(isGatewayRestartResult == 0x00){
							Thread.sleep(1000);
						}else{
							
							if(isGatewayRestartResult == NORMAL){
								flag = true;
							}
	 ...
	 */
    @Override
	public boolean notifySystemRebooting(NotifyEvent evt){
    	...
    	synchronized(writeLock){
			...
			if(storeCD.equals(evt.getStoreCD()) && gwID.equals(evt.getGwID())){
				...
				try{
					...
					isSystemRebootingResult = 0x00;
					
					write(evt.getData(), false);
					
					for(short i = 0; i < 20; i++){
						
						if(isSystemRebootingResult == 0x00){
							Thread.sleep(1000);
						}else{
							
							if(isSystemRebootingResult == NORMAL){
								flag = true;
							}
	 ...
	 */
    @Override
    public byte[] notifyChangeIFServer(NotifyEvent evt){
    	...
    	synchronized(writeLock){
			...
			if(storeCD.equals(evt.getStoreCD()) && gwID.equals(evt.getGwID())){
				...
				try{
					...
					isIFServerChangeStat = 0x00;
					ifServerChangeResult = null;
					
					write(evt.getData(), false);
					
					for(short i = 0; i < 20; i++){
						
						if(isIFServerChangeStat == 0x00){
							Thread.sleep(1000);
						}else{
							
							if(isIFServerChangeStat == NORMAL){
								result = ifServerChangeResult;
							}
	 ...
	 */
    @Override
	public boolean notifyResetIOBoard(NotifyEvent evt){
		...
		synchronized(writeLock){
			...
			if(storeCD.equals(evt.getStoreCD()) && gwID.equals(evt.getGwID())){
				...
				try{
					...
					isIOBoardResetResult = 0x00;
					
					write(evt.getData(), false);
					
					for(short i = 0; i < 20; i++){
						
						if(isIOBoardResetResult == 0x00){
							Thread.sleep(1000);
						}else{
							
							if(isIOBoardResetResult == NORMAL){
								flag = true;
							}
                		...
                		try{
        					...
        					if(stx == STX && etx == ETX){
        						...
        						if(dataCRC == chekCRC){
        							...
        							// IF Server 주소변경 결과 수신
        							else if(cmd == 0x10){
        								
        								logger.info("IF Server 주소변경 결과 수신");
        								
        								isIFServerChangeStat = dataBuffer[1];
        								ifServerChangeResult = data;
        							}
    ...
    private void doProcessCmd1(byte cmd, byte[] dataBuffer) throws Exception {
		...
		logger.info("GW Version=" + version);
		
		// I/O Version
		byte ioVersionByte = dataBuffer[idx++];
		
		bd1 = new BigDecimal(ioVersionByte);
		
		float ioVersion = bd1.divide(bd2).floatValue();
		logger.info("I/O Version=" + ioVersion);
		
		// IP
		byte[] ips  = Utils.getBytes(dataBuffer, 3, 8);
		// MAC
		byte[] macs = Utils.getBytes(dataBuffer, 11, 12);
		...
		// 매장코드
		storeCD = new String(Utils.getBytes(dataBuffer, 23, 20)).trim();
		logger.info("STR CD=" + storeCD);
		
		// GW ID
		gwID = String.valueOf(ByteUtils.toUnsignedShort(Utils.getBytes(dataBuffer, 43, 2)));
		...
		if("".equals(storeCD)){
			...
			if(gwMstMap != null){
				...
				if(!macAddress.equals(StringUtils.defaultIfEmpty(gwMstMap.get("GW_ADDR"), "")) || 
						...
						!String.valueOf(version).equals(StringUtils.defaultIfEmpty(gwMstMap.get("GW_SW_VER"), "")) ||
						!String.valueOf(ioVersion).equals(StringUtils.defaultIfEmpty(gwMstMap.get("GW_SW_VER2"), ""))){
					...
					paramMap.put("GW_SW_VER2", String.valueOf(ioVersion));
		...
		// 결과 전송
		short wIdx = 0;
		byte[] writeBuffer = new byte[89];
		writeBuffer[wIdx++] = STX;
		
		byte[] lenBytes = ByteUtils.toUnsignedShortBytes(0x55);
		writeBuffer[wIdx++] = lenBytes[0];
		writeBuffer[wIdx++] = lenBytes[1];
		...
		if(gwMstMap != null){
	        ...
	        if(gwMstMap.get("OFF_TIME") == null){
	        	...
	        	StringTokenizer st = new StringTokenizer(gwMstMap.get("OFF_TIME"), ":");
	        ...
	        if(gwMstMap.get("ON_TIME") == null){
	        	...
	        	StringTokenizer st = new StringTokenizer(gwMstMap.get("ON_TIME"), ":");
    ...
    private void doProcessCmd2(byte cmd, byte[] dataBuffer) throws Exception {
    	...
    	try{
			...
			if(storeCD.equals(dStoreCD) && dGwID.equals(gwID)){
				...
				jsonObj.put("COMM_STATUS", String.valueOf(commStatus));
    ...
    private void doProcessCmd8(byte cmd, byte[] dataBuffer) throws Exception {
		...
		if(result == NORMAL){
			...
			byte ctlAgt    = dataBuffer[idx++]; // 제어주체(0x00:SEMS, 0x01:Man)
			byte weatherCD = dataBuffer[idx++]; // 날씨코드
			...
			paramMap.put("WEATHER_CD", String.valueOf(weatherCD));
    ...
    private void doProcessCmd10(byte cmd, byte[] dataBuffer) throws Exception {
    	...
    	logger.info("sensingTemp="+sensingTemp);
    	logger.info("controlTemp="+controlTemp);
    	logger.info("guideTemp="+guideTemp);
    	logger.info("controlWho="+controlWho);
    	logger.info("controlGubun="+controlGubun);
    	logger.info("onOFF="+onOFF);
    	logger.info("coolNheat="+coolNheat);
    	logger.info("controlAgt="+controlAgt);
		...
		paramMap.put("COOLHEAT", coolNHeatConvert);
		...
		if(cmd == 0x0A){
			...
			logger.info("냉난방 온도제어결과 수신 응답 전송");
    ...
    private void doProcessCmd13(byte cmd, byte[] dataBuffer) throws Exception {
    	...
    	logger.info("시간동기화 수신");
    	
		logger.info("시간동기화 수신 응답 전송");
		...
		byte[] lenBytes = ByteUtils.toUnsignedShortBytes(0x0A);
		writeBuffer[wIdx++] = lenBytes[0];
		writeBuffer[wIdx++] = lenBytes[1];
		writeBuffer[wIdx++] = cmd;
    ...
    private void doProcessCmd15(byte cmd, byte[] dataBuffer) throws Exception {
		...
		isSystemRebootingResult = result;
    }
    ...
    private void doProcessCmd17(byte cmd, byte[] dataBuffer) throws Exception {
		...
		// 결과 전송
		short wIdx = 0;
		byte[] writeBuffer = new byte[42];
		writeBuffer[wIdx++] = STX;
		
		byte[] lenBytes = ByteUtils.toUnsignedShortBytes(0x26);
		writeBuffer[wIdx++] = lenBytes[0];
		writeBuffer[wIdx++] = lenBytes[1];
