...
public class NotifyServer extends Thread{
	 ...
	 * @param file
	 */
	public boolean notifyFileUpdate(String storeCD, String gwID, byte[] data){
		...
		if(listener != null)
			result = listener.notifyFileUpdate(evt);
	 ...
	 * @return
	 */
	public byte[] notifyChangeIFServer(String storeCD, String gwID, byte[] data){
		...
		if(listener != null)
			result = listener.notifyChangeIFServer(evt);
	 ...
	 * @return
	 */
	public boolean notifyResetIOBoard(String storeCD, String gwID, byte[] data){
		...
		if(listener != null)
			result = listener.notifyResetIOBoard(evt);
	...
	class Worker implements Runnable{
					...
					if(stx == STX && etx == ETX){
						...
						if(dataCRC == chekCRC){
							...
							// IF Server 주소변경 수신
								...
								doProcessCMD16(cmd, dataBuffer, readBuffer, readBytes);
							}
							// 게이트웨이 상태 요청
							else if(cmd == 0x12){
							...
							// I/O 보드 리셋 수신
								...
								doProcessCMD19(cmd, dataBuffer, readBuffer, readBytes);
							}
						}else{
		...
		private void doProcessCMD4(byte cmd, byte[] dataBuffer, byte[] readBuffer, int readBytes) throws Exception {
			...
			if(result == NORMAL){
				...
				flag = notifyFileUpdate(storeCD, String.valueOf(gwID), data);
			}
		 ...
		 * @throws Exception
		 */
		private void doProcessCMD16(byte cmd, byte[] dataBuffer, byte[] readBuffer, int readBytes) throws Exception {
			
			logger.info("IF Server 주소변경 요청");
			...
			byte[] result = notifyChangeIFServer(storeCD, String.valueOf(gwID), data);
		 ...
		 * @throws Exception
		 */
		private void doProcessCMD19(byte cmd, byte[] dataBuffer, byte[] readBuffer, int readBytes) throws Exception {
			
			logger.info("I/O 보드 리셋 수신");
			...
			boolean flag = notifyResetIOBoard(storeCD, String.valueOf(gwID), data);
